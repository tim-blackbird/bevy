...
fn animate_translation(
    ...
    for mut transform in &mut query {
        transform.translation.x = 100.0 * time.elapsed_secs().sin() - 400.0;
        transform.translation.y = 100.0 * time.elapsed_secs().cos();
    }
}
...
fn animate_rotation(
    ...
    for mut transform in &mut query {
        transform.rotation = Quat::from_rotation_z(time.elapsed_secs().cos());
    }
}
...
fn animate_scale(
    ...
    for mut transform in &mut query {
        ...
        transform.scale = Vec3::splat((time.elapsed_secs().sin() + 1.1) * 2.0);
    }
}
